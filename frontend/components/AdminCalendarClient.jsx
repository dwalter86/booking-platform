...
export default function AdminCalendarClient({ resources = [], unavailabilityBlocks = [], initialView = 'timeGridWeek' }) {
  ...
  // Block details panel — keeps the existing right-column behaviour
  const [selectedEvent, setSelectedEvent] = useState(null);

  // Booking slide-over panel — new (replaces the old booking right-column)
  const [selectedBookingId, setSelectedBookingId] = useState(null);
  const [bookingPanelMode, setBookingPanelMode] = useState('view');
  ...
  // Refetch bookings for the current calendar view — called after the panel
  // does anything that might change a booking (save / confirm / cancel).
    ...
    <>
            ...
            <div className="card-body p-0">
              {!mounted ? null : <FullCalendarComponent
                ...
                eventClick={(info) => {
                  ...
                  if (!raw) return;
                  if (type === 'booking') {
                    setSelectedBookingId(raw.id);
                    setBookingPanelMode('view');
                  } else {
                    setSelectedEvent({ type, raw });
                  }
                }}
                datesSet={(info) => {
                  const api = calendarRef.current?.getApi();
      ...
      {selectedBookingId && (
        <BookingPanel
          ...
          onModeChange={setBookingPanelMode}
          onClose={() => {
