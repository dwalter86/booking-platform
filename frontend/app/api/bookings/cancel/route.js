...
export async function POST(request) {
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.redirect(new URL(`/bookings?error=${encodeURIComponent(data?.error || 'Unable to cancel booking.')}`, baseUrl), 302);
    }

    return NextResponse.redirect(new URL('/bookings?success=Booking%20cancelled.', baseUrl), 302);
  } catch {
