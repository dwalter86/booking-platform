...
export async function POST(request) {
  ...
  try {
    ...
    if (!response.ok && response.status !== 404) {
      return NextResponse.redirect(new URL(`${redirectBase}&error=Unable%20to%20delete%20exception`, baseUrl), 302);
    }
  } catch {
    return NextResponse.redirect(new URL(`${redirectBase}&error=API%20unavailable`, baseUrl), 302);
  }

  return NextResponse.redirect(new URL(`${redirectBase}&success=Exception%20deleted`, baseUrl), 302);
}
