...
export async function POST(request) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const message = encodeURIComponent(data?.error || 'Unable to update exception');
  ...
  return NextResponse.redirect(new URL(`${redirectBase}${sep(redirectBase)}success=Exception%20updated`, baseUrl), 302);
}
